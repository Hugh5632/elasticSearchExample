...
namespace elasticlient {
    ...
    Client &Client::operator=(const Client &client) {
        if(this == &client)
            return *this;
        this->impl->hostUrlList.assign(client.impl->hostUrlList.begin(),client.impl->hostUrlList.end());
        return *this;
    }
...
//        LOG(LogLevel::INFO, "Host returned %ld in %lf s for %s.", response.status_code,response.elapsed, entireUrl.c_str());
...
//    void Client::SetValue(std::vector<std::string> &host_url_list, int32_t timeout) {
//        this->timeout_ = timeout;
//        this->host_url_list_ = host_url_list;
//
//    }
...
//    std::string doc_str{"{}"};
//    cpr::Response index_response = client.index("test_index","_doc","",doc_str);
